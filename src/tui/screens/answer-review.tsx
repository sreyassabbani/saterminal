...
type ReviewPane = "question" | "review";
...
type ReviewLine = ChoiceLine & {
  kind?: "heading" | "explanation" | "notice";
};
...
export function AnswerReviewScreen({ title, question, answer, attempt, resultDetail, action }: AnswerReviewScreenProps) {
  ...
  const contextHeight = answerContextHeight(question, attempt, resultDetail, paneWidth);
  const reviewHeight = Math.max(5, viewportHeight - contextHeight - 1);
  const pageSize = Math.max(4, viewportHeight - 2);
  const [activePane, setActivePane] = useState<ReviewPane>("review");
  ...
  useInput((input, key) => {
    if (action.kind === "next" && (input === "n" || key.return)) action.run();
    else if (action.kind === "back" && key.escape) action.run();
  ...
  const verdict = attempt.outcome.toUpperCase();
  const footerAction = action.kind === "next" ? "enter/n next" : "esc back";

  return (
    <Screen
      title={title}
      detail={<Text bold color={verdictColorMap[attempt.outcome]}>{verdict}</Text>}
      footer={`tab/←/→ pane · j/k line · [/] page · ${footerAction}`}
    >
      <Box flexDirection={sideBySide ? "row" : "column"} gap={3}>
        ...
        {(sideBySide || activePane === "review") && (
          <Box width={paneWidth} flexDirection="column">
            ...
            <Box marginTop={1}>
              <AnswerContext question={question} attempt={attempt} resultDetail={resultDetail} />
            </Box>
            <Box marginTop={1}>
              <ReviewContent
                question={question}
...
function AnswerContext({ question, attempt, resultDetail }: { question: Question; attempt: Attempt; resultDetail: ResultDetail }) {
  const showDifficulty = resultDetail !== "brief";
  const showTaxonomy = resultDetail === "detailed";
  return (
    <Box flexDirection="column">
      <Text>
        <Text bold color="cyan">{formatDuration(attempt.durationSeconds)}</Text>
...
function ReviewContent({ question, answer, width, height, scroll }: {
  question: Question;
  answer?: string;
  width: number;
  height: number;
  ...
  const selected = question.choices.findIndex((choice) => choice.key === answer);
  ...
  const lines: ReviewLine[] = [
    ...answer === undefined ? [
      { text: "Selected answer unavailable for this earlier attempt.", kind: "notice" as const },
      { text: " " },
    ] : [],
    ...choiceLines,
    { text: " " },
  ...
  return (
    <Box flexDirection="column">
      {lines.slice(start, start + height).map((line, index) => {
        ...
        const chosen = choice?.key === answer;
        const color = line.kind === "heading" ? "cyan" : line.kind === "notice" ? "gray" : correct ? "green" : chosen ? "red" : undefined;
        return (
...
function answerContextHeight(question: Question, attempt: Attempt, resultDetail: ResultDetail, width: number): number {
  const showDifficulty = resultDetail !== "brief";
  const showTaxonomy = resultDetail === "detailed";
  const lines = [
    `${formatDuration(attempt.durationSeconds)}${showDifficulty ? `  ·  ${difficultyLabels[question.difficulty]} difficulty` : ""}`,
    ...showTaxonomy ? [`${question.domain}  ${domainLabels[question.domain]}`, `${question.skill}  ${skillLabels[question.skill]}`] : [],
  ];
  return lines.reduce((total, line) => total + Math.max(1, Math.ceil(Bun.stringWidth(line) / width)), 0);
}
